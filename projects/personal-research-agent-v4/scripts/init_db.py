#!/usr/bin/env python3
"""Create the Personal Research Agent v4 SQLite database."""
...
def main() -> None:
    ...
    db_path = args.db_path or config.runtime_db_path
    db.initialize_database(db_path)
    seeded = [] if args.skip_users else db_users.seed_users_from_config(args.users, db_path)
    if db_path and not str(db_path).strip().lower().startswith(("postgres://", "postgresql://")):
        resolved_path = db.get_db_path(db_path)
        print(f"initialized_db={resolved_path}")
    else:
        target = str(db_path).strip() if db_path else str(config.database_url or "")
        print(f"initialized_db=postgres:{target}")
