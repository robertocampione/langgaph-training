#!/usr/bin/env python3
"""Deterministic validation for the v4 SQLite persistence layer."""
...
def main() -> None:
    ...
    parser.add_argument("--runtime", action="store_true", help="Validate configured runtime backend (postgres/sqlite).")
    ...
    if args.runtime or config.db_backend == "postgres":
        validate(config.runtime_db_path)
        target = config.database_url if config.db_backend == "postgres" else config.db_path
        print(f"db_validation=pass runtime=true backend={config.db_backend} target={target}")
        return
